...
ConVar g_hAllowAnnounce,
	   g_hAllowForceKillAnnounce, g_hAllowPrintLiveTime, g_hMissionFailedAnnounce, g_hEnableHealthSet, g_hSurvivorLimit, g_hHealthLimit, g_hAllowPrintZeroDamage;
...
public void OnPluginStart()
{
	...
	g_hEnableHealthSet		  = CreateConVar("tank_damage_enable_healthset", "0", "是否将坦克的生命值设置为 z_tank_health 数值", CVAR_FLAG, true, 0.0, true, 1.0);
...
void printTankDamage(int client)
{
	...
	/* 显示标题 */
	if (IsClientInGame(client) && !IsFakeClient(client)) { CPrintToChatAll("[{green}!{default}] {blue}生还者对 {green}Tank {default}({green}%N{default}) {blue}的伤害统计", client); }
	else {
	...
	{
		if (IsClientInGame(client) && !IsFakeClient(client)) { CPrintToChatAll("[{green}!{default}] {green}%N {blue}存活时间：{green}%s", getTime(tankLiveTime[client])); }
		else {
	...
	for (i = 0; i < survivorIndex; i++)
	{
		...
		else if (!g_hAllowPrintZeroDamage.BoolValue && damage == 0) {
			continue;
		}
	}
}
...
{
	return elem1[1] > elem2[1] ? -1 : elem1[1] == elem2[1] ? elem1[0] > elem2[0] ? -1 : elem1[0] == elem2[0] ? 0
																											 : 1
														   : 1;
}
