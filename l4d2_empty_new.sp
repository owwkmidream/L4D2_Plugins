...
void initLog()
{
	...
	IsSaveMessage("--=============================================================--");
	SaveFormatMessage("插件已加载");
}

void SaveFormatMessage(const char[] message)
{
	char Msg[256], Time[32];
	FormatTime(Time, sizeof(Time), "%Y-%m-%d %H:%M:%S", -1);
	Format(Msg, sizeof(Msg), "%s %s", Time, message);
...
public void OnClientDisconnect(int client)
{
	...
	char Msg[256];
	Format(Msg, sizeof Msg, "玩家退出事件触发玩家：%N", client);
	SaveFormatMessage(Msg);
	CreateTimer(1.0, PlayerDisconnectTimer, _);
}

public Action PlayerDisconnectTimer(Handle hTiemr)
{
	if (IsEmptyServer())
	{
		if (g_hTimer_CheckEmpty == null)
			g_hTimer_CheckEmpty = CreateTimer(20.0, Timer_CheckEmptyServer, INVALID_HANDLE, TIMER_REPEAT);
		SaveFormatMessage("服务器最后一个玩家退出，开始计时");
	}
...
public Action Timer_CheckEmptyServer(Handle timer, any param)
{
	...
	if (IsEmptyServer())
	{
		...
		{	 // Idle for 50s
			...
			SaveFormatMessage("服务器运行100秒没有玩家，即将进入处理");

			switch (g_iEmptySwitch)
			{
				case 1:
				{
					SaveFormatMessage("重启服务器");
					IsDetermineSystemType(L4D_GetServerOS());	 //判断系统类型:0=windows,1=linux.
				}
				case 2:
				{
					char Msg[256];
					Format(Msg, sizeof(Msg), "更改当前地图为 %s", g_sEmptyCode);
					SaveFormatMessage(Msg);
...
bool IsEmptyServer()
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		{
			char Msg[256];
			Format(Msg, sizeof Msg, "检查：存在玩家：%N", client);
			SaveFormatMessage(Msg);
			return false;	 // Someone is in the server}
		}
	}

	SaveFormatMessage("检查：服务器为空");
	return true;
}
...
void IsRecordLogContent(int iType)
{
	...
	if (g_iEmptyLog == 1)
	{
		char Msg[256], Time[32];
		FormatTime(Time, sizeof(Time), "%Y-%m-%d %H:%M:%S", -1);
		Format(Msg, sizeof(Msg), "服务器没人了:[%s],系统类型:%s,服务器类型:%s", Time, iType == 0 ? "windows" : iType == 1 ? "linux"
																														  : "其它",
			   IsDedicatedServer() ? "专用" : "本地");
...
void IsSaveMessage(const char[] Message)
{
	...
	if (fileHandle == null)
	{														   // 检查文件是否打开成功
		LogAction(0, -1, "无法打开日志文件: %s", chatFile);	   // 将错误消息写入服务器日志
	}
	else
	{
