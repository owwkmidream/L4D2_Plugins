...
public int ChampVoteHandler(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_Display) {
		...
		if (percent < limit) {
			...
			PrintToChatAll("\x04[提示]\x05%t\x04[\x03%d\x04,\x03%d\x04,\x03%d\x04]", "Vote Successful", RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
...
public void OnPluginStart() {
	...
	g_hCVar_VotingAdDelayTime = CreateConVar("l4d2_acs_voting_ad_delay_time", "1.0", "救援关幸存者离开开始区域后提示预选下一关地图的投票指令.", FCVAR_NOTIFY);
	g_hCVar_VotingAdDelayfrequency = CreateConVar("l4d2_acs_voting_ad_delay_frequency", "1", "救援关预选下一关地图的投票指令显示多少次.", FCVAR_NOTIFY);
	...
	g_hCVar_NextMapAdInterval = CreateConVar("l4d2_acs_next_map_ad_interval", "0.0", "救援关循环公告下一张地图信息的时间(秒).", FCVAR_NOTIFY);
...
public Action Timer_AdvertiseNextMap(Handle timer, any param) {
	...
	if(g_hCVar_NextMapAdMode.IntValue != DISPLAY_MODE_DISABLED)	{
		if (OnFinaleOrScavengeMap())
			DisplayNextMapToAll();
		if (g_hCVar_NextMapAdInterval.FloatValue > 0) CreateTimer(g_hCVar_NextMapAdInterval.FloatValue, Timer_AdvertiseNextMap, _, TIMER_FLAG_NO_MAPCHANGE);
	}
