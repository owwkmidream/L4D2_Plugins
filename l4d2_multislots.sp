...
int		g_iAway, g_iKick, g_iSset, g_iMaxs, g_iLimit, g_iTeam, g_iAutobot, g_iAllInRestart;
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_idle", CommandIdle, "快速闲置指令.");
	...
	g_hGive1		= CreateConVar("l4d2_multislots_Survivor_spawn1", "0", "启用给予主武器. 0=禁用, 1=启用(随机获得:冲锋枪,消音冲锋枪).", CVAR_FLAGS);
	g_hGive2		= CreateConVar("l4d2_multislots_Survivor_spawn2", "3", "启用给予副武器. 0=禁用, 1=启用(随机获得:小手枪,马格南,斧头), 2=斧头., 3=手枪", CVAR_FLAGS);
	...
	g_hAway			= CreateConVar("l4d2_multislots_enabled_away", "1", "启用指令 !away 强制加入旁观者. 0=禁用, 1=启用(公共), 2=启用(只限管理员).", CVAR_FLAGS);
	...
	g_hAutobot		= CreateConVar("l4d2_multislots_enabled_auto_bot", "1", "是否允许指令!kb禁止开局添加电脑? 0=禁用 1=启用.", CVAR_FLAGS);
	g_hAllInRestart = CreateConVar("l4d2_multislots_All_in_restart", "1", "所有玩家都进入游戏后是否重启当前章节? 0=禁用 1=启用.", CVAR_FLAGS);
	...
	g_hAutobot.AddChangeHook(IsOtherConVarChanged);
	g_hAllInRestart.AddChangeHook(IsOtherConVarChanged);
...
public void IsLoadGameCFG()
{
	...
	if (FileExists(sPath))
	{
		...
		if (PrepSDKCall_SetFromConf(hGameData, SDKConf_Signature, "ScenarioRestart") == false)
			SetFailState("[提示] 找不到 Signature: \"Restart\"签名.");
		else
		{
			PrepSDKCall_AddParameter(SDKType_PlainOldData, SDKPass_Plain);
			hRestart = EndPrepSDKCall();
			if (hRestart == null)
				SetFailState("[提示] 创建 SDKCall: \"Restart\" 失败.");
		}

		StartPrepSDKCall(SDKCall_Player);
		if (PrepSDKCall_SetFromConf(hGameData, SDKConf_Signature, "GoAwayFromKeyboard") == false)
			SetFailState("[提示] 找不到 Signature: \"GoAwayFromKeyboard\"签名.");
		else
		{
			hGoAwayFromKeyboard = EndPrepSDKCall();
			if (hGoAwayFromKeyboard == null)
				SetFailState("[提示] 创建 SDKCall: \"GoAwayFromKeyboard\" 失败.");
		}
...
void vInitPatchs(GameData hGameData = null)
{
	...
	m_fRoundEndInterval = hGameData.GetOffset("CDirector::m_fRoundEndInterval");
	if (m_fRoundEndInterval == -1)
		SetFailState("Failed to find offset:  CDirector::m_fRoundEndInterval");
}
...
public void OnMapStart()
{
	...
	if (g_iAllInRestart == 1)
		CreateTimer(1.0, RestartRoundTimer, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);
}
...
void IsGetOtherCvars()
{
	...
	g_iAutobot		   = g_hAutobot.IntValue;
	g_iAllInRestart	   = g_hAllInRestart.IntValue;
...
void GiveWeapon(int client)
{
	switch (g_iGive2)
	{
		...
		case 3:
		{
			BypassAndExecuteCommand(client, "give", "pistol");	  //斧头.
		}
	}
	switch (g_iGive3)
	{
...
public Action CheckClientState(Handle Timer, DataPack hPack)
{
	...
	bool ClientTakeOverBot = hPack.ReadCell();
	{
		...
		if (TotalFreeBots() > 0)
		{
			...
			if (bot <= 0)
				Isdead = IsGetTakeOverTarget();

			if (ClientTakeOverBot)
				TakeOverBot(client, true, Isdead);	  //更改为 true 则自动加入幸存者,反之是闲置状态.
			else
				TakeOverBot(client, false, Isdead);	   //更改为 true 则自动加入幸存者,反之是闲置状态.
...
void TakeOverBot(int client, bool completely, bool IsDead)
{
	if (!IsClientInGame(client))
		return;
...
public Action CheckAndKillPlayerTimer(Handle hTimer, int userid)
{
...
void KillPlayerTimer(int userid)
{
	int client = GetClientOfUserId(userid);
...
public Action RestartRoundTimer(Handle hTimer)
{
	if (!AreAllInGame())
		return Plugin_Continue;

	static const float restartTimeSec = 5.0;
	PrintToChatAll("\x04[提示]\x05所有玩家载入完成，游戏将在\x03%d\x05秒后重启.", RoundFloat(restartTimeSec));
	CreateTimer(restartTimeSec, RestartGameTimer, _, TIMER_FLAG_NO_MAPCHANGE);

	return Plugin_Stop;
}

public Action RestartGameTimer(Handle hTimer)
{
...
void vSpawnCheck()
{
	...
	// 启用kb后和autobot是否添加电脑.
	// Autobot == 1 and gbKickBots == true 就不添加电脑.
	if (g_iAutobot == 0 || !gbIsKickBot)
	{
...
public Action Command_Respawn(int client, int args)
{
	...
	if (IsValidClient(client) && !IsFakeClient(client) && !IsPlayerAlive(client))
	{
		...
		// 找到一个活着的玩家，并将刚复活的玩家传送到该玩家的位置
		for (int i = 1; i <= MaxClients; i++)
		{
			if (i != client && IsValidClient(i) && !IsFakeClient(i) && IsPlayerAlive(i))
			{
				float Origin[3];
...
public Action Command_RestartRound(int client, any args)
{
	if (!bCheckClientAccess(client))
	{
		PrintToChat(client, "\x05[失败] \x04你无权使用!restart指令");
		return Plugin_Handled;
	}
...
public Action CommandIdle(int client, any args)
{
	if (!bCheckClientAccess(client))
	{
		PrintToChat(client, "\x05[失败] \x04你无权使用!idle指令");
		return Plugin_Handled;
	}
