...
public void OnPluginStart()
{
	...
	// COMMANDS
	// ====================================================================================================
	RegConsoleCmd("sm_progress", CmdProgress, "手动触发进度提醒。");
	RegConsoleCmd("sm_pg", CmdProgress, "手动触发进度提醒。");


	// ====================================================================================================
	// CVARS
	// ====================================================================================================
	g_hCvarAllow =		CreateConVar(	"l4d_coop_markers_allow",			"1",					"0 =插件关闭，1 =插件开启。", CVAR_FLAGS );
	if( !g_bLeft4Dead2 )
		g_hCvarModes =	CreateConVar(	"l4d_coop_markers_modes",			"5",					"打开这些游戏模式中的插件. 0=All, 1=Coop, 2=Survival, 4=Versus. 将数字添加在一起.", CVAR_FLAGS );
	g_hCvarPanel =		CreateConVar(	"l4d_coop_markers_panel",			"1",					"0=打印到聊天面板, 1=打印到控制面板", CVAR_FLAGS );
	g_hCvarPercent =	CreateConVar(	"l4d_coop_markers_percent",			"25",					"进度提醒的百分比。", CVAR_FLAGS );
	g_hCvarTimer =		CreateConVar(	"l4d_coop_markers_timer",			"2.0",					"计时器检查进度的时间间隔。", CVAR_FLAGS );
	CreateConVar(						"l4d_coop_markers_version",			PLUGIN_VERSION,			"Coop Markers 插件版本.", FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
//					COMMANDS
// ====================================================================================================
public Action CmdProgress(int client, int args)
{

    int currentProgress = CalculateCurrentProgress();
    FireMarker(currentProgress);

    return Plugin_Handled;
}


stock int CalculateCurrentProgress()
{
    float dist;
    int total;
    int area;
    ...
    {
        dist /= total;
        return RoundToCeil(dist / g_fDistance * 100);
    }
