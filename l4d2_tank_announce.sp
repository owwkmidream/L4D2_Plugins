...
public void OnPluginStart()
{
	...
	g_hTankPrompt	 = CreateConVar("l4d2_tank_prompt", "1", "设置坦克出现时的提示类型(启用多个就把数字相加). 0=禁用, 1=聊天窗, 2=屏幕中下.", CVAR_FLAGS);
	g_hTankMultiple	 = CreateConVar("l4d2_tank_Multiples", "0.8;1.0;2.0;2.5", "设置游戏难度对应的倍数(留空=使用默认值:1.0).", CVAR_FLAGS);
...
void SetTankHealth(int client, float Multiples, char[] sName)
{
	...
	// helpeer不调整
	if (StrContains(GetSurvivorName(client, false), "Helper", false) == -1)
	{
		// PrintToChatAll("触发调整");
	...
	{	 // 是helper
		PrintToChatAll("触发调整");
		SetClientHealth(client, GetConVarInt(FindConVar("l4d_tank_throw_tank_health")));
	}
}
