...
{
	name		= "坦克丢特感",
	author		= "Pan Xiaohai & HarryPotter & Owwkmmidream",
	description = "Tanks throw special infected instead of rock",
	version		= "2.1.1h-2023/11/24",
...
public void OnPluginStart()
{
	...
	l4d_tank_throw_tank			= CreateConVar("l4d_tank_throw_tank", "3.0", "Tank 的权重[0.0, 10.0]", FCVAR_NOTIFY, true, 0.0, true, 10.0);
	...
	l4d_tank_throw_witch		= CreateConVar("l4d_tank_throw_witch", "3.0", "Witch 的权重[0.0, 10.0]", FCVAR_NOTIFY, true, 0.0, true, 10.0);
	...
	l4d_tank_throw_smoker_limit = CreateConVar("l4d_tank_throw_smoker_limit", "2", "场上 Smoker 的上限[1 ~ 5] (如果达到上限，投掷 Smoker 队友，如果所有 Smokers 都在忙，投掷 Tank 自身)", FCVAR_NOTIFY, true, 1.0, true, 5.0);
	l4d_tank_throw_boomer_limit = CreateConVar("l4d_tank_throw_boomer_limit", "2", "场上 Boomer 的上限[1 ~ 5] (如果达到上限，投掷 Boomer 队友)", FCVAR_NOTIFY, true, 1.0, true, 5.0);
	if (L4D2Version)
	{
		l4d_tank_throw_charger_limit = CreateConVar("l4d_tank_throw_charger_limit", "2", "场上 Charger 的上限[1 ~ 5] (如果达到上限，投掷 Charger 队友，如果所有 Chargers 都在忙，投掷 Tank 自身)", FCVAR_NOTIFY, true, 1.0, true, 5.0);
		l4d_tank_throw_spitter_limit = CreateConVar("l4d_tank_throw_spitter_limit", "2", "场上 Spitter 的上限[1 ~ 5] (如果达到上限，投掷 Spitter 队友)", FCVAR_NOTIFY, true, 1.0, true, 5.0);
		l4d_tank_throw_jockey_limit	 = CreateConVar("l4d_tank_throw_jockey_limit", "2", "场上 Jockey 的上限[1 ~ 5] (如果达到上限，投掷 Jockey 队友，如果所有 Jockeys 都在忙，投掷 Tank 自身)", FCVAR_NOTIFY, true, 1.0, true, 5.0);
	}
	l4d_tank_throw_tank_limit  = CreateConVar("l4d_tank_throw_tank_limit", "3", "场上 Tank 的上限[1 ~ 10] (如果达到上限，投掷 Tank 队友或自身)", FCVAR_NOTIFY, true, 1.0, true, 10.0);
	l4d_tank_throw_witch_limit = CreateConVar("l4d_tank_throw_witch_limit", "3", "场上 Witch 的上限[1 ~ 10] (如果达到上限，投掷 Tank 自身)", FCVAR_NOTIFY, true, 1.0, true, 10.0);
...
public void L4D_TankRock_OnRelease_Post(int tank, int rock, const float vecPos[3], const float vecAng[3], const float vecVel[3], const float vecRot[3])
{
	...
	if ((IsFakeClient(tank) && random <= fl4d_tank_throw_si_ai) || (!IsFakeClient(tank) && random <= fl4d_tank_throw_si_real))
	{
...
void   GetGameData()
{
	...
	if (hGameConf != null)
	{
		...
		if (offset == -1)
		{
			SetFailState("Unable to find NextBotPointer offset.");
			return;
		}
		...
		if (g_hNextBotPointer == null)
		{
			SetFailState("Cannot initialize NextBotPointer SDKCall, signature is broken.");
			return;
		}
		...
		if (offset == -1)
		{
			SetFailState("Unable to find GetLocomotion offset.");
			return;
		}
		...
		if (g_hGetLocomotion == null)
		{
			SetFailState("Cannot initialize GetLocomotion SDKCall, signature is broken.");
			return;
		}
		...
		if (offset == -1)
		{
			SetFailState("Unable to find Jump offset.");
			return;
		}
		StartPrepSDKCall(SDKCall_Raw);
		PrepSDKCall_SetVirtual(offset);
		...
		if (g_hJump == null)
		{
			SetFailState("Cannot initialize Jump SDKCall, signature is broken.");
			return;
		}
	}
	else
