...
#define PLUGIN_AUTHOR				"sorallll, reworked by B[R]UTUS, owwkmidream"
#define PLUGIN_DESCRIPTION			"提供了在合作模式下自定义Boss产卵percents的可能性"
...
{
	InitGameData();
	g_cvShowBossPercRules = CreateConVar("l4d2_cbs_show_boss_percent_difficulties", "4", "展示各难度下boss生成的百分比:0 - 在任何难度下都不显示 | 1 - 简单难度 | 2 - 普通及以下难度 | 3 - 高级及以下难度 | 4 - 专家及以下难度。");
...
public Action CurrentCmd(int client, int args)
{
	...
	CPrintToChatAll("{green}[{default}Current{green}]{default}: {blue}%d%%", GetCurrentSurvivorsMapCompletion());
	return Plugin_Handled;
}
...
public Action DebugCmd(int client, int args)
{
	...
	if (!L4D_IsMissionFinalMap())
	{
		...
		CPrintToChat(client, "{green}[{default}调试信息{green}]{default}: 地图 {blue}%s", mapName);
		CPrintToChat(client, "{green}->{default} max_tanks: {blue}%i", g_iMaxTanks);

		for (int i = 1; i <= g_iMaxTanks; i++)
		{
			if (g_iTankSpawnChance[i - 1] == -1)
				CPrintToChat(client, "{green}->{default} Tank #{olive}%i{default} -> {blue}事件产生", i, g_iTankSpawnChance[i - 1], g_iTankMinSpawnPercent[i - 1], g_iTankMaxSpawnPercent[i - 1]);
			else
