...
public void OnPluginStart() {
	...
	g_hCVar_VotingEnabled = CreateConVar("acs_voting_system_enabled", "1", "启用玩家投票换图插件? 0=禁用, 1=启用.", _, true, 0.0, true, 1.0);
	g_hCVar_VoteWinnerSoundEnabled = CreateConVar("acs_voting_sound_enabled", "1", "打开投票换图菜单时播放声音? 0=禁用, 1=启用.", _, true, 0.0, true, 1.0);
	g_hCVar_VotingAdMode = CreateConVar("acs_voting_ad_mode", "2", "救援关幸存者离开开始区域后更换下一张地图的方式,只显示一次(选项1和2是提示玩家输入指令打开投票菜单). 0=禁用, 1=屏幕中下, 2=聊天窗, 3=自动打开投票菜单.\n * Note: This is only displayed once during a finale or scavenge map *", _, true, 0.0, true, 3.0);
	g_hCVar_VotingAdDelayTime = CreateConVar("acs_voting_ad_delay_time", "1.0", "救援关幸存者离开开始区域后提示预选下一关地图的投票指令.\n * Note: If the server is up, changing this in the .cfg file takes two map changes before the change takes place *", _, true, 0.1, false);
	g_hCVar_NextMapMenuOptions = CreateConVar("acs_next_map_menu_options", "0", "救援关投票换图列表里地图的显示类型. 0=官方和附加地图, 1=仅限官方图, 2=当前地图的类型.");
	g_hCVar_NextMapMenuExcludes = CreateConVar("acs_next_map_menu_excludes", "0", "救援关投票换图列表里排除当前地图? 0=不排除, 1=排除当前地图.");
	g_hCVar_NextMapMenuOrder= CreateConVar("acs_next_map_menu_order", "0", "救援关投票菜单中地图的显示顺序. 0=官方图,然后是附加地图, 1=随机.");
	g_hCVar_NextMapAdMode = CreateConVar("acs_next_map_ad_mode", "2", "救援关公告下一张地图信息的显示方式. 0=禁用, 1=屏幕中下, 2=聊天窗.", _, true, 0.0, true, 2.0);
	g_hCVar_NextMapAdInterval = CreateConVar("acs_next_map_ad_interval", "0.0", "救援关循环公告下一张地图信息的时间(秒).", _, true, 60.0, false);
	g_hCVar_MaxFinaleFailures = CreateConVar("acs_max_coop_finale_failures", "0", "战役模式救援关幸存者任务失败多少次后自动换图? 0=禁用", _, true, 0.0, false);
	g_hCVar_ChMapPolicy =  CreateConVar("acs_chmap_policy", "2", "启用 !chmap 和 !chmap2 投票换图指令?\n 0=禁用(同时禁用公告投票换图指令).\n 1=启用(不投票视为不同意). \n 2=启用(不投票视为同意)");	
	g_hCVar_ChMapBroadcastInterval =  CreateConVar("acs_chmap_broadcast_interval", "180.0", "设置循环公告投票指令 \"!chmap\" 的时间(秒). 0=禁用.");	
