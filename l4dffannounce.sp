		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i))
			{
				...
				switch(AnnounceType.IntValue)
				{
					...
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
							CPrintToChat(attackerc, "[{olive}FF{default}] %T", "FF_dealt (C)", attackerc, DamageCache[attackerc][i], victim);
						if (IsClientInGame(i) && !IsFakeClient(i))
							CPrintToChat(i, "[{olive}FF{default}] %T", "FF_receive (C)", i, attacker, DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
